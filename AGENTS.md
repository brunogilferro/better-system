...
| Layer | Rule | Example |
|-------|------|---------|
| Files | `snake_case` English, mirroring the DB table name hierarchy | `project_table.ts`, `project_table_participant.ts` |
| Classes | `PascalCase` English, mirroring the DB table name hierarchy | `class ProjectTable`, `class ProjectTableParticipant` |
...
DB table names encode the parent-child relationship through prefixes (e.g. `Projetos_Mesas_Participantes` belongs to `Projetos_Mesas` which belongs to `Projetos`). Model files, classes, and raw query types **must mirror this hierarchy** in English:
...
Without this, Lucid's manyToMany `preload()` generates `WHERE pivot_fk IN ('123')` (text bind) which may not match an `integer` FK column.
